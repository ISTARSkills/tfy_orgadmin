...
public class UIUtils {
	...
	//	hashMap.put("asoctrainername", listOfassociateTrainer);
		...
		//	hashMap.put("associate_trainee", res.get(0).get("associate_trainee").toString());
			...
			String orderedLessonInCourse ="SELECT 	cmsession_module.cmsession_id AS cm_id FROM 	lesson_cmsession,	cmsession_module,   module_course WHERE 	module_course.module_id = cmsession_module.module_id AND cmsession_module.cmsession_id = lesson_cmsession.cmsession_id AND module_course .course_id = "+res.get(0).get("course_id")+"";
			 //ViksitLogger.logMSG(this.getClass().getName(),orderedLessonInCourse);
			...
			if(res.get(0).get("status").toString().equalsIgnoreCase("ASSESSMENT")){
				...
				//ViksitLogger.logMSG(this.getClass().getName(),"ASSESSMENT");
	...
	public StringBuffer getAllTrainer(ArrayList<Integer> selectedTrainer) {
		// <option value="">Data Analytics</option>
		String sql = "SELECT 	istar_user. ID, 	istar_user.email, 	user_profile.first_name FROM 	istar_user, 	user_role,   user_profile WHERE 	istar_user. ID = user_role.user_id AND user_role.role_id in (select id from role where role_name in ('TRAINER','MASTER_TRAINER')) AND user_profile.user_id = istar_user. ID";
	...
	public JSONArray getCourseReportEvent(int org_id, int course_id, String type) {
		...
		// //ViksitLogger.logMSG(this.getClass().getName(),"101 -> " + sql);
		DBUTILS db = new DBUTILS();
		List<HashMap<String, Object>> data = db.executeQuery(sql);
		//ViksitLogger.logMSG(this.getClass().getName(),"102 -> " + data.size());
		...
		for (HashMap<String, Object> item : data) {
			...
			if (eventdate != null) {
				...
				if (item.get("event_name") != null){}
					//ViksitLogger.logMSG(this.getClass().getName(),"Event not added due to dateformat issue " + item.get("event_name").toString());
			}
		}
		JSONArray arr_strJson = new JSONArray(course_event_list);
		// //ViksitLogger.logMSG(this.getClass().getName(),arr_strJson);
