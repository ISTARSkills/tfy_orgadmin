...
public class OrgAdminDashboardServices {
	...
	public List<HashMap<String, Object>> getStudentAttendanceStatus(String eventId) {
		String sql2 = "SELECT 	attendance.status, 	user_profile.first_name, 	CASE WHEN user_profile.profile_image LIKE 'null' OR user_profile.profile_image IS NULL THEN 	'http://api.talentify.in/video/android_images/' || UPPER ( 		SUBSTRING (user_profile.first_name FROM 1 FOR 1) 	) || '.png' ELSE 	'http://api.talentify.in/' || user_profile.profile_image END AS profile_image,  ( 	SELECT 		COUNT (*) 	FROM 		attendance 	WHERE 		attendance.event_id = "+eventId+" 	AND status = 'ABSENT' ) AS ABSENT,  ( 	SELECT 		COUNT (*) 	FROM 		attendance 	WHERE 		attendance.event_id = "+eventId+" 	AND status = 'PRESENT' ) AS present FROM 	attendance, 	user_profile WHERE 	attendance.event_id = "+eventId+" AND attendance.user_id = user_profile.user_id";
		DBUTILS dbutils = new DBUTILS();
		List<HashMap<String, Object>> logs = dbutils.executeQuery(sql2);
		return logs;
	}
