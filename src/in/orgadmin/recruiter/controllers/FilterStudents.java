...
public class FilterStudents extends IStarBaseServelet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*HashMap<String, String> filtersKeyValue= new HashMap<>();
		Enumeration<String> parameterNames = request.getParameterNames();
		while (parameterNames.hasMoreElements()) {
			String paramName = parameterNames.nextElement();
			String paramValue = request.getParameter(paramName);
			filtersKeyValue.put(paramName, paramValue);
		}*/
		/*param_name -> filtered_cities[] : param_value ->ADSD
		param_name -> filtered_colleges[] : param_value ->2
		param_name -> filtered_ug_degrees[] : param_value ->Bachelor of Applied Sciences(B.A.S)
		param_name -> filtered_pg_degrees[] : param_value ->Doctor of Medicine(M.D)
		param_name -> skill_selector[] : param_value ->2688
		param_name -> vacancy_idd : param_value ->2000*/
...
//		List<Student> students = new StudentDAO().findAll();
	...
	private String getFilterSQL(HttpServletRequest request) {
		...
		sb.append("select distinct student.id from student, student_profile_data, pincode, address, college,	jobs_event where jobs_event.actor_id=student.id and jobs_event.vacancy_id="+vacancy_id+" and student.id = student_profile_data.student_id and student.address_id = address.id and address.pincode_id = pincode.id and student.organization_id = college.id and student.user_type='STUDENT'");
	...
	private String getPGSearchPart(HttpServletRequest request) {
		...
		String pg_degree_value=" and student_profile_data.post_graduation_specialization_name in (";
	...
	private String getUGSearchPart(HttpServletRequest request) {
		...
		String ug_degree_value=" and student_profile_data.under_graduation_specialization_name in (";
