	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameterMap().containsKey("org_id"))
		{
			...
			else if (request.getParameterMap().containsKey("batch_id"))
			{
				...
				getEvent="SELECT 	bse.eventdate AS START, 	bse.eventdate + ( 		( 			(bse.eventhour * 60) + bse.eventminute 		) * INTERVAL '1 minute' 	) AS END,  bse.event_name AS event_name,  CAST (bse. ID AS VARCHAR(50)) AS ID,  ( 	(bse.eventhour * 60) + bse.eventminute ) AS duration,  bse.status AS status,  org. ID AS org_id,  org. NAME AS orgname,  s. first_name AS trainer_name,  bse.batch_group_id AS batch_group_id,  s. user_id AS trainer_id,  CD.classroom_identifier AS classroom,  CD. ID AS class_id,  BG. NAME AS batch_name,  BG. NAME || ', ' || s. first_name AS title,  'Batch Name: ' || BG. NAME || '<br/>Org Name: ' || org. NAME || ' <br/>Classroom (ID): Room No. ' || CD.classroom_identifier || '.<br/>Duration: ' || ( 	(bse.eventhour * 60) + bse.eventminute ) || ' mins.<br/> Event Time: ' || bse.eventdate || '<br/>Trainer: ' || s. first_name || ' <br/> Status: ' || bse.status AS description FROM 		user_profile s, user_role ur, 	batch_schedule_event bse, 	 	batch_group BG, 	classroom_details CD, 	organization org WHERE 	CD. ID = bse.classroom_id  AND bse.batch_group_id = BG. ID AND BG.college_id = org. ID AND bse.actor_id = s.user_id AND s.user_id = ur.user_id AND ur.role_id = 14 AND event_name NOT LIKE '%TEST%' AND bse. TYPE LIKE '%EVENT_TRAINER%' AND bse.batch_group_id =(select batch_group_id from batch where id="+batch_id+") and bse.course_id = (SELECT 		course_id 	FROM 		batch 	WHERE 		ID = "+batch_id+")";
			...
			else
			{
				...
				String org_id = request.getParameter("org_id");
				getEvent="SELECT 	bse.eventdate AS START, 	bse.eventdate + ( 		( 			(bse.eventhour * 60) + bse.eventminute 		) * INTERVAL '1 minute' 	) AS END,  bse.event_name AS event_name,  CAST (bse. ID AS VARCHAR(50)) AS ID,  ( 	(bse.eventhour * 60) + bse.eventminute ) AS duration,  bse.status AS status,  org. ID AS org_id,  org. NAME AS orgname,  s.first_name AS trainer_name,  bse.batch_group_id AS batch_id,  s. user_id AS trainer_id,  CD.classroom_identifier AS classroom,  CD. ID AS class_id,  BG. NAME AS batch_name,  BG. NAME || ', ' || s.first_name AS title,  'Batch Name: ' || BG. NAME || '<br/>Org Name: ' || org. NAME || ' <br/>Classroom (ID): Room No. ' || CD.classroom_identifier || '.<br/>Duration: ' || ( 	(bse.eventhour * 60) + bse.eventminute ) || ' mins.<br/> Event Time: ' || bse.eventdate || '<br/>Trainer: ' || s. first_name || ' <br/> Status: ' || bse.status AS description FROM 	user_profile s, user_role ur, 	batch_schedule_event bse, 	 	batch_group BG, 	classroom_details CD, 	organization org WHERE 	CD. ID = bse.classroom_id AND  bse.batch_group_id = BG. ID AND BG.college_id = org. ID AND bse.actor_id = s.user_id AND s.user_id = ur.user_id AND ur.role_id = 14 AND event_name NOT LIKE '%TEST%' AND bse. TYPE LIKE '%EVENT_TRAINER%' AND org. ID = "+org_id;
		...
		{
			//getting all events
			getEvent="SELECT 	bse.eventdate AS START, 	bse.eventdate + ( 		( 			(bse.eventhour * 60) + bse.eventminute 		) * INTERVAL '1 minute' 	) AS END,  bse.event_name AS event_name,  CAST (bse. ID AS VARCHAR(50)) AS ID,  ( 	(bse.eventhour * 60) + bse.eventminute ) AS duration,  bse.status AS status,  org. ID AS org_id,  org. NAME AS orgname,  s.first_name AS trainer_name,  bse.batch_group_id AS batch_id,  s. user_id AS trainer_id,  CD.classroom_identifier AS classroom,  CD. ID AS class_id,  BG. NAME AS batch_name,  BG. NAME || ', ' || s.first_name AS title,  'Batch Name: ' || BG. NAME || '<br/>Org Name: ' || org. NAME || ' <br/>Classroom (ID): Room No. ' || CD.classroom_identifier || '.<br/>Duration: ' || ( 	(bse.eventhour * 60) + bse.eventminute ) || ' mins.<br/> Event Time: ' || bse.eventdate || '<br/>Trainer: ' || s. first_name || ' <br/> Status: ' || bse.status AS description FROM 	user_profile s, user_role ur, 	batch_schedule_event bse, 	batch_group BG, 	classroom_details CD, 	organization org WHERE 	CD. ID = bse.classroom_id  AND bse.batch_group_id = BG. ID AND BG.college_id = org. ID AND bse.actor_id = s.user_id AND s.user_id = ur.user_id AND ur.role_id = 14 AND event_name NOT LIKE '%TEST%' AND bse. TYPE LIKE '%EVENT_TRAINER%'";
