...
public class ReportDetailService {
	...
	public List<HashMap<String, Object>> getAllSessions(int batch_id, int offset, boolean flag) {
		...
		String sql = "SELECT 	CAST(event_session_log.event_id as varchar), 	event_session_log.cmsession_id, 	cmsession.title,bse.eventdate FROM 	event_session_log, 	cmsession,batch_schedule_event bse WHERE 	event_id IN ( 		SELECT 			ID 		FROM 			batch_schedule_event 		WHERE 			batch_id = "
				+ batch_id
				+ " 	) AND event_session_log.cmsession_id = cmsession. ID AND bse. ID = event_session_log.event_id GROUP BY 	event_session_log.event_id, 	event_session_log.cmsession_id, 	cmsession.title,bse.eventdate ORDER BY bse.eventdate desc limit 3 offset "
				+ offset;

		if (flag) {
			sql = "SELECT 	CAST(event_session_log.event_id as varchar), 	event_session_log.cmsession_id, 	cmsession.title,bse.eventdate FROM 	event_session_log, 	cmsession,batch_schedule_event bse WHERE 	event_id IN ( 		SELECT 			ID 		FROM 			batch_schedule_event 		WHERE 			batch_id = "
					+ batch_id
					+ " 	) AND event_session_log.cmsession_id = cmsession. ID AND bse. ID = event_session_log.event_id GROUP BY 	event_session_log.event_id, 	event_session_log.cmsession_id, 	cmsession.title,	bse.eventdate ORDER BY bse.eventdate desc";
		}
	...
	public List<HashMap<String, Object>> getAllStudents(int batch_id) {
		String sql = "SELECT DISTINCT 	s. ID AS student_id, 	s. NAME AS NAME, 	s.email, 	CASE WHEN sp.profile_image LIKE 'null' OR sp.profile_image IS NULL THEN 	'http://api.talentify.in/video/android_images/' || UPPER (SUBSTRING(s. NAME FROM 1 FOR 1)) || '.png' ELSE 	'http://api.talentify.in/' || sp.profile_image END AS profile_image FROM 	batch b, 	batch_students bs, 	batch_group bg, 	student s, 	student_profile_data sp WHERE 	bs.student_id = s. ID AND bg. ID = b.batch_group_id AND bg. ID = bs.batch_group_id AND sp.student_id = s. ID AND b. ID = "
				+ batch_id + " ORDER BY 	s. NAME";
