...
public class TaskCardFactoryRecruitment {
	public StringBuffer showTrainerProfileCard(int trainerId) {
		...
		sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-3 no-padding'><b>Gender</b></div>" + "<div class='col-md-8	 no-padding'>" + user.getUserProfile().getGender() + "</div></div>");
		sb.append("</li>");
		sb.append("<li class='list-group-item' style='margin-left: -16px;     margin-right: -13px;'	>                                                   ");
		sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-3 no-padding'><b>UG Degree</b></div>" + "<div class='col-md-8	 no-padding'>" + user.getProfessionalProfile().getUnderGraduateDegreeName() + "</div></div>");
		sb.append("</li>");
		sb.append("<li class='list-group-item' style='margin-left: -16px;     margin-right: -13px;'	>                                                   ");
		sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-3 no-padding'><b>PG Degree</b></div>" + "<div class='col-md-8	 no-padding'>" + user.getProfessionalProfile().getPgDegreeName() + "</div></div>");
		sb.append("</li>");
		sb.append("<li class='list-group-item' style='margin-left: -16px;     margin-right: -13px;'	>                                                   ");
		sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-3 no-padding'><b>Experience</b></div>" + "<div class='col-md-8	 no-padding'>" + user.getProfessionalProfile().getExpereinceInYears() + " years " + user.getProfessionalProfile().getExperienceInMonths() + " months</div></div>");
		sb.append("</li>");
		...
		sb.append("<li class='list-group-item' style='margin-left: -16px;     margin-right: -13px;'	>                                                   ");
		sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-3 no-padding'><b>Interested  Courses</b></div>" + "<div class='col-md-7	 no-padding'>" + interestedCourses + "</div></div>");
		sb.append("</li>");

		sb.append("<li class='list-group-item' style='margin-left: -16px;     margin-right: -13px;'	>                                                   ");
		sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-8 no-padding'><h3>Available Days and Time Slots</h3></div>" + "<div class='col-md-3	 no-padding'></div></div>");
		sb.append("</li>");
		...
		for (HashMap<String, Object> row : timeSlots) {
			sb.append("<li class='list-group-item' style='margin-left: -16px;     margin-right: -13px;'	>                                                   ");
			sb.append("<div class='row'>" + "<div class='col-md-1  no-padding'></div>" + "<div class='col-md-3 no-padding'><b>" + row.get("day") + "</b></div>" + "<div class='col-md-8	 no-padding'>"
					+ row.get("time_slots").toString().replaceAll("- 9am, 9am -", "-").replaceAll("- 10am, 10am -", "-").replaceAll("- 11am, 11am -", "-").replaceAll("- 12pm, 12pm -", "-").replaceAll("- 1pm, 1pm -", "-").replaceAll("- 2pm, 2pm -", "-").replaceAll("- 3pm, 3pm -", "-").replaceAll("- 4pm, 4pm -", "-").replaceAll("- 5pm, 5pm -", "-").replaceAll(", $", "") + "</div></div>");
			sb.append("</li>");
		}
	...
	public StringBuffer showSummaryCard(int trainerID) {
		...
		sb.append(" <div class='task-complete-header' style='background-color:#eb384f;    color: white;'>                 ");
		sb.append(" <h6 class='p-xxs font-normal bg-muted m-l-xs m-t-none'> Placement Summary</h6>                                                 ");
		sb.append(" <h3 class='p-xxs m-l-xs' style='padding:20px;'>" + data.size() + " courses applied.</h3>               ");
		sb.append(" </div>                                                        ");
		...
		for (HashMap<String, Object> row : data) {
			...
			String getstatus = "select stage, empanelment_status, TP.first_name as trainer_name, IP.first_name as interviewer_name, created_at  from (select created_at, stage, empanelment_status, course_id, trainer_id from trainer_empanelment_status where course_id = " + row.get("id") + " and trainer_id =" + trainerID
			...
			String title = "The Candidate was <strong>" + StringUtils.capitalize(status) + "</strong>  in stage " + stage + " for the position of " + "Trainer - <strong>" + StringUtils.capitalize(row.get("course_name").toString()) + " " + score + " " + interviewerName + "</strong>  on " + format.format(date) + " </p";
	...
	public StringBuffer showCourseCard(int trainerID, int courseID, int interviewerid) {
		...
		for(int i=0; i< stages.size(); i++)
		{
			...
			if (stages.get(i).equalsIgnoreCase(TrainerEmpanelmentStageTypes.TELEPHONIC_INTERVIEW)) {
				...
				String sql = "select * from trainer_empanelment_status where trainer_id = " + trainerID
						+ " and course_id = " + course.getId() + " and stage = 'L2'";
				...
				if (stagesCleared.get(i - 1)) {
					...
					String sql = "select * from trainer_empanelment_status where trainer_id = " + trainerID
							+ " and course_id = " + course.getId() + " and stage = '" + stages.get(i) + "'";
					List<HashMap<String, Object>> statusData = util.executeQuery(sql);
					String score = "";
					...
					if (statusData.size() > 0) {
						...
						title = "The Candidate was <strong>"
								+ StringUtils.capitalize(statusData.get(0).get("empanelment_status").toString())
								+ "</strong>  in stage " + stages.get(i) + " for the position of "
								+ "Trainer - <strong>" + StringUtils.capitalize(course.getCourseName()) + " " + score
								+ " " + interviewerName + "</strong>  on " + format.format(date) + " </p";
						if (statusData.get(0).get("empanelment_status").toString()
								.equalsIgnoreCase(TrainerEmpanelmentStatusTypes.SELECTED)) {
		...
		sb.append("<div class='ibox-title'><h5>Feedback</h5> "
				+ "<div class='ibox-tools'><span class='label label-info pull-right reverse_view'><i class='fa fa-exchange'></i></span></div>"
				+ " </div>                                          ");
		sb.append("<div class='ibox-content' id='ibox-content' style='margin-bottom:20px;padding:10px;'>");
		...
		if (!util.executeQuery(sql1).get(0).get("empanelment_status").toString().equalsIgnoreCase("REJECTED")) {
			String stage = util.executeQuery(sql1).get(0).get("stage").toString();
			switch (stage) {
			case "L3":
				String sql2 = "select id, interview_skill_name from interview_skill where course_id = "+courseID+" and stage_type='L4'";
				List<HashMap<String, Object>> items = util.executeQuery(sql2);
				for (HashMap<String, Object> item : items) {
					sb.append("<div class='row'><div class='col-md-8'>"+item.get("interview_skill_name")+"</div>");
					sb.append(
							"<div class='rateYo col-md-4' data-course_id='"+courseID+"' data-user_id='"+trainerID+"' data-skill_id='"+item.get("id")+"' "
									+ " data-interviewer_id='"+interviewerid+"' data-stage='L4' ></div> </div>                                                               ");
				}
				sb.append("<textarea rows='4' cols='50' style='margin-top:10px;' id='comments_"+trainerID+"_"+courseID+"' tool-tip=comments here>");
				sb.append("</textarea>                                                                              ");
				sb.append("<label class='checkbox-inline'><input type='checkbox' value='SELECTED' id='selected_'"+trainerID+"_"+courseID+">Selected</label>");
				sb.append("<button data-_holer_id='trainer_rating_"+trainerID+"_"+courseID+"' class='btn btn-primary submit_feedback pull-right btn-xs' data-course_id='"+courseID+"' data-user_id='"+trainerID+"' "
								+ " data-interviewer_id='"+interviewerid+"' type='button'>Submit</button>");
				break;
			...
			case "L4":
				...
				for (HashMap<String, Object> item : items1) {
					...
					sb.append(
							"<div class='rateYo col-md-4' data-course_id='"+courseID+"' data-user_id='"+trainerID+"' data-skill_id='"+item.get("id")+"' "
									+ " data-interviewer_id='"+interviewerid+"' data-stage='L5' ></div> </div>                                                               ");
