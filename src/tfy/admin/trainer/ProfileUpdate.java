...
public class ProfileUpdate extends IStarBaseServelet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		underGraduationSpecializationName = request.getParameter("underGraduationSpecializationName") != null
				? request.getParameter("underGraduationSpecializationName")
				: "";
		underGradutionMarks = request.getParameter("underGradutionMarks") != null
				? Float.parseFloat(request.getParameter("underGradutionMarks"))
				: 0f;
		postGraduationSpecializationName = request.getParameter("postGraduationSpecializationName") != null
				? request.getParameter("postGraduationSpecializationName")
				: "";
		postGradutionMarks = request.getParameter("postGradutionMarks") != null
				? Float.parseFloat(request.getParameter("postGradutionMarks"))
				: 0f;
		...
		preferredLocation = request.getParameter("preferredLocation") != null
				? request.getParameter("preferredLocation")
				: "";
		isStudyingFurtherAfterDegree = request.getParameter("isStudyingFurtherAfterDegree") != null
				? Boolean.getBoolean(request.getParameter("isStudyingFurtherAfterDegree"))
				: false;
		areaOfInterest = request.getParameter("areaOfInterest") != null ? request.getParameter("areaOfInterest") : "";
		interestedInTypeOfCourse = request.getParameter("interestedInTypeOfCourse") != null
				? request.getParameter("interestedInTypeOfCourse")
				: "";
		below_poverty_line = request.getParameter("below_poverty_line") != null
				? Boolean.parseBoolean(request.getParameter("below_poverty_line"))
				: false;
		...
		if (!userType.equalsIgnoreCase("")) {
			...
			if (batchCode != null && !batchCode.equalsIgnoreCase("")) {
				...
				for (HashMap<String, Object> row : grpData) {
					...
					String checkIfMappingExist = "select cast(count(*) as integer) as cnt from batch_students where student_id ="
							+ userId + " and batch_group_id=" + groupId + "";
					List<HashMap<String, Object>> mappins = util.executeQuery(checkIfMappingExist);
					if (mappins.size() > 0 && (int) mappins.get(0).get("cnt") == 0) {
						String insertIntoBatchStudents = "insert into batch_students (id, student_id, batch_group_id) values((select COALESCE(max(id),0)+1 from batch_students),"
								+ userId + "," + groupId + ")";
						util.executeUpdate(insertIntoBatchStudents);

						String checkIfExistInOrgMapping = "select cast(count(*) as integer) as cnt from user_org_mapping where user_id = "
						...
						if (orgMappins.size() > 0 && (int) orgMappins.get(0).get("cnt") == 0) {
							String insertIntoUserOrg = "INSERT INTO user_org_mapping (user_id, organization_id, id) VALUES ("
									+ userId + ", " + orgId
									+ ", ((select COALESCE(max(id),0)+1 from user_org_mapping)));";
			...
			if (request.getParameterMap().containsKey("email")) {
				String updateUserPassword = "UPDATE istar_user SET password='" + password + "', mobile='" + mobile
						+ "' WHERE id=" + user.getId();
			...
			String updateUserProfile = "update user_profile set address_id=" + address_id + ", first_name='" + firstName
					+ "', last_name='" + lastName + "', dob='" + dob + "', gender='" + gender + "' ,aadhar_no= "
					+ aadharno + " ,place_of_birth='" + place_of_birth + "',father_name='" + father_name + "'"
					+ ",caste_category='" + caste_category + "',religion='" + religion + "' where user_id =" + userId;
			util.executeUpdate(updateUserProfile);

			String sql = "SELECT cast (count(*) as INTEGER) as is_exist  FROM professional_profile WHERE user_id = "
					+ userId;
			List<HashMap<String, Object>> professional_profile_isexist_check = util.executeQuery(sql);
			if (professional_profile_isexist_check.size() > 0 && (int)professional_profile_isexist_check.get(0).get("is_exist") == 0) {
				...
				String insertIntoProfessionalProfile = "INSERT INTO professional_profile (id, user_id, has_under_graduation,has_post_graduation, under_graduate_degree_name, pg_degree_name, experience_in_years, experince_in_months, pan_no, yop_10, marks_10, yop_12, marks_12, under_graduation_specialization_name, under_gradution_marks, post_graduation_specialization_name, post_gradution_marks, is_studying_further_after_degree, job_sector, preferred_location, company_name, position, duration, description, interested_in_type_of_course, area_of_interest,below_poverty_line) VALUES ((select COALESCE(max(id),0)+1 from professional_profile), "
						+ userId + ", '" + Boolean.toString(hasUgDegree).charAt(0) + "','"
				...
				String updateProfessionalProfile = "update professional_profile set has_under_graduation='"
						+ Boolean.toString(hasUgDegree).charAt(0) + "'" + " ,has_post_graduation = '"
						+ Boolean.toString(hasPgDegree).charAt(0) + "', under_graduate_degree_name='" + ugDegree + "'"
						+ ", pg_degree_name='" + pgDegree + "', experience_in_years='" + experinceYears
						+ "', experince_in_months='" + experinceMonths + "' " + ", pan_no = '" + panNo + "', yop_10="
						+ yop10 + ", marks_10=" + marks10 + ", yop_12=" + yop12 + ", marks_12=" + marks12 + ","
						+ " under_graduation_specialization_name='" + underGraduationSpecializationName
						+ "', under_gradution_marks=" + underGradutionMarks + "," + " post_graduation_specialization_name='"
						+ postGraduationSpecializationName + "', post_gradution_marks=" + postGradutionMarks + ","
						+ " is_studying_further_after_degree='" + Boolean.toString(isStudyingFurtherAfterDegree).charAt(0)
						+ "', job_sector='" + jobSector + "'," + " preferred_location='" + preferredLocation
						+ "', company_name='" + companyName + "', position='" + position + "', duration=" + duration + ","
						+ " description='" + description + "', interested_in_type_of_course='" + interestedInTypeOfCourse
						+ "', area_of_interest='" + areaOfInterest + "'," + "below_poverty_line='"
						+ Boolean.toString(below_poverty_line).charAt(0) + "' where user_id =" + userId + "";
			...
			String findAlreadyInterestedCourse = "select distinct course_id from trainer_intrested_course where trainer_id = "
					+ userId;
				...
				for (String course_id : courses) {
					...
					if (!interstedCourseList.contains(courseId)) {
						...
						for (HashMap<String, Object> assess : assessments) {
							...
							String notificationTitle = "An assessment with title <b>" + assessment.getAssessmenttitle()
									+ "</b> of course <b>" + course.getCourseName()
									+ "</b> has been added to task list.";
							...
							int taskId = taskService.createTodaysTask(taskTitle.trim().replace("'", ""),
									taskDescription.trim().replace("'", ""), 300 + "", userId + "", assessmentId + "",
									"ASSESSMENT");
							notificationService.createIstarNotification(300, userId, notificationTitle,
									notificationDescription, "UNREAD", null, NotificationType.ASSESSMENT, true, taskId,
				...
				try {
					...
					for (Object obja : obj.keySet()) {
						...
						String ssql = "INSERT INTO trainer_available_time_sloat ( 	ID, 	trainer_id, 	DAY, 	t8am_9am, 	t9am_10am, 	t10am_11am, 	t11am_12pm, 	t12pm_1pm, 	t1pm_2pm, 	t2pm_3pm, 	t3pm_4pm, 	t4pm_5pm, 	t5pm_6pm ) VALUES 	( 		(SELECT COALESCE(max(id)+1,1) FROM trainer_available_time_sloat), 	  "
								+ userId + ", 		'" + day + "', 		'" + t8am_9am + "', 		'" + t9am_10am
								+ "', 		'" + t10am_11am + "', 		'" + t11am_12pm + "', 		'" + t12pm_1pm
