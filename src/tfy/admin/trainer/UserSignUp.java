		 ...
		 {			 
			 TaskServices taskService = new TaskServices();
	           String sql = "INSERT INTO address ( 	ID, 	addressline1, 	addressline2, 	pincode_id, 	address_geo_longitude, 	address_geo_latitude ) VALUES 	( 		(SELECT max(id)+1 FROM address), 		'"+addressLine1+"', 		'"+addressLine2+"', 		 (select id from pincode where pin="+pincode+"), 		 NULL, 		 NULL 	)RETURNING ID;";
	            ...
	            String insertIntoIstarUser ="INSERT INTO istar_user (id, email, password, created_at, mobile, auth_token, login_type, is_verified) VALUES ((select COALESCE(max(id),0)+1 from istar_user), '"+email+"', '"+password+"', now(), "+mobile+", NULL, NULL, 't') returning id;";
	    		int urseId = db.executeUpdateReturn(insertIntoIstarUser);
	    		if(userType.equalsIgnoreCase("TRAINER")){
	    			String insertPresentorIntoIstarUser ="INSERT INTO istar_user (id, email, password, created_at, mobile, auth_token, login_type, is_verified) VALUES ((select COALESCE(max(id),0)+1 from istar_user), '"+presentor_email+"', '"+password+"', now(), "+mobile+", NULL, NULL, 't') returning id;";
	    			int presentorId = db.executeUpdateReturn(insertPresentorIntoIstarUser);
	    		}
	    		
	    		String createUserProfile ="INSERT INTO user_profile (id,  first_name, last_name,  gender, user_id,address_id ,dob) VALUES ((select COALESCE(max(id),0)+1 from user_profile), '"+firstName+"', '"+lastName+"', '"+gender+"',"+urseId+","+address_id+" , '"+dob+"');";
	    		db.executeUpdate(createUserProfile);
	
	    		String insertIntoUserRole = "INSERT INTO user_role (user_id, role_id, id, priority) VALUES ("+urseId+", (select id from role where role_name='"+userType+"'), ((select COALESCE(max(id),0)+1 from user_role)), 1);";
	    		db.executeUpdate(insertIntoUserRole);
	    		
	    		String insertIntoProfessionalProfile ="INSERT INTO professional_profile (id, user_id, has_under_graduation,has_post_graduation, under_graduate_degree_name, pg_degree_name, experience_in_years, experince_in_months) VALUES ((select COALESCE(max(id),0)+1 from professional_profile), "+urseId+", '"+Boolean.toString(hasUgDegree).charAt(0)+"','"+Boolean.toString(hasPgDegree).charAt(0)+"','"+ugDegree+"','"+pgDegree+"','"+experinceYears+"','"+experinceMonths+"'); ";
	    		...
	    		if(userType.equalsIgnoreCase("TRAINER"))
	    		{
		    		...
		    		String groupNotificationCode = UUID.randomUUID().toString();
		    		 if(!courseIds.equalsIgnoreCase("")){
		    			 ...
		    			 {
		    				 String insertInIntrestedTable = "insert into trainer_intrested_course (id, trainer_id, course_id) values((select COALESCE(max(id),0)+1 from trainer_intrested_course),"+urseId+","+course_id+")";
		    				 ...
		    				 for(HashMap<String, Object> assess: assessments)
		    				 {
		    					...
		    					int taskId = taskService.createTodaysTask(taskTitle.trim().replace("'", ""), taskDescription.trim().replace("'", ""), 300+"",urseId+"" , assessmentId+"", "ASSESSMENT");
		    					IstarNotification istarNotification = notificationService.createIstarNotification(300, urseId, notificationTitle, notificationDescription, "UNREAD", null, NotificationType.ASSESSMENT, true, taskId, groupNotificationCode);
		    				 }
		    			 }
		    			 ...
		    			 if(defaultAssessment.size()>0)
		    			 {
		    				 ...
		    				 if(defAssessments!=null)
		    				 {
		    					 for(String defAssess : defAssessments.split(","))
		    					 {
		    						 ...
		    						 if(assessment!=null)
		    						 {
		    	    					String notificationTitle = "An assessment with title <b>"+assessment.getAssessmenttitle()+"</b> has been added to task list.";
		    	    					String notificationDescription =  notificationTitle;
		    	    					String taskTitle = assessment.getAssessmenttitle();
		    	    					String taskDescription = notificationDescription;
		    	    					int taskId = taskService.createTodaysTask(taskTitle.trim().replace("'", ""), taskDescription.trim().replace("'", ""), 300+"",urseId+"" , aid+"", "ASSESSMENT");
		    	    					IstarNotification istarNotification = notificationService.createIstarNotification(300, urseId, notificationTitle, notificationDescription, "UNREAD", null, NotificationType.ASSESSMENT, true, taskId, groupNotificationCode);
		    						 }
		            ...
		            try {
		    				...
		    				for(Object obja:obj.keySet()){
		    					...
		    					String ssql = "INSERT INTO trainer_prefred_location ( 	ID, 	trainer_id, 	marker_id, 	prefred_location ) VALUES 	((SELECT COALESCE(max(id)+1,1) FROM trainer_prefred_location), "+urseId+", '"+obja.toString()+"', '"+obj.get(obja).toString()+"');";
