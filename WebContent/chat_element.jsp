...
{
	String getListOfOrg = "SELECT 	user_role. user_id as id, 	role.role_name, 	organization. NAME, 	org_type, 	( 		CASE 		WHEN image IS NULL THEN 			'video/android_images/' || SUBSTRING (TRIM(NAME) FROM 1 FOR 1) || '.png' 		ELSE 			image 		END 	) AS image, 	COUNT (chat_messages. ID) AS chat_count FROM 	organization, 	chat_messages, 	user_org_mapping, 	user_role, role WHERE user_role.role_id = role.id and organization. ID = user_org_mapping.organization_id AND user_org_mapping.user_id = user_role.user_id AND user_role.role_id in ( 8,15) AND ( 	user_role.user_id = chat_messages.user_id 	OR user_role.user_id = chat_messages.receiver_id ) AND chat_messages.sent = 'f' GROUP BY 	user_role. user_id, 	organization. NAME, 	org_type, 	image, role_name order by name; ";
	orgList = util.executeQuery(getListOfOrg);		

	String getBatchGroups="select batch_group.id , batch_group.name, 'video/android_images/'||substring(trim(batch_group.name) from 1 for  1) || '.png' as image, count(batch_group_messages.id) as chat_count  from batch_group, batch_group_messages where batch_group.id = batch_group_messages.batch_group_id and batch_group_messages.sent='f' GROUP BY batch_group.id , batch_group.name, image";
