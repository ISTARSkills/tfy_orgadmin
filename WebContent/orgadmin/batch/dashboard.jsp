							...
							<div class="col-lg-12">
						        <div class="ibox float-e-margins">
									<div class="ibox-title">
										<h5>Attendance associated with this batch</h5>
										<div class="ibox-tools">
										<a href="<%=baseURL%>orgadmin/batch/update_attendance.jsp?batch_id=<%=batch.getId() %>" class="btn btn-primary btn-xs right">Update Attendance</a>
											<a class="collapse-link">
												<i class="fa fa-chevron-up"></i>
											</a>
										</div>
									...
									<div class="ibox-content">
										<%
											...
											conditions.put("batch_id", batch_id+"");
										%>
										<%=(new DatatableUtils()).getReport(300, conditions, ((IstarUser) request.getSession().getAttribute("user")), "NONE").toString()%>
									</div>
						        </div>
						    </div>
						</div>
							...
							<div class="col-lg-12">
						        <div class="ibox float-e-margins">
									<div class="ibox-title">
										...
										<div class="ibox-tools">
										<a target="_new" href="<%=baseURL %>orgadmin/events/create_events.jsp?batch_id=<%=batch.getId()%>" class="btn btn-primary btn-xs right">Create Classroom Event</a>		 &nbsp; &nbsp;
										<a target="_new" href="<%=baseURL %>orgadmin/events/create_assessment.jsp?batch_id=<%=batch.getId()%>" class="btn btn-primary btn-xs right">Create Assessment Event</a>		 &nbsp; &nbsp;
										
										<% if(batch.getBatchGroup().getName().contains("ISTAR -Trainer-Assessment")){ %>
										<a target="_new" href="<%=baseURL %>orgadmin/events/create_trainer_assessment.jsp?batch_id=<%=batch.getId()%>" class="btn btn-primary btn-xs right">Create Trainer Assessment Event</a>		 &nbsp; &nbsp;
										<%} %>
...
$('#calendar').fullCalendar({
	...
	events : [ 
<%List<HashMap<String, Object>> res = bu.getEventPerBatch(batch);
int count = 0;
