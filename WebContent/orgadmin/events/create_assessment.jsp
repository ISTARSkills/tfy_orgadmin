...
<%
	...
	BatchScheduleEventDAO dao1 = new BatchScheduleEventDAO();
	BatchScheduleEvent be = new BatchScheduleEvent();
	
	Batch b = new BatchDAO().findById(batch_id);	
	OrgadminUtil orgutil = new OrgadminUtil();
	ArrayList<HashMap<String, String>>  assess_details = orgutil.getAssessmentListForBatch(b);
							...
							<%
							 if(assess_details.size()>0)
							 {
								 ...
								 %>
								  ...
								  <div class="col-sm-6 ">
                                    <div>
                                        <label> <input type="checkbox" class="i-checks" value="<%=id%>" name="assessment_id"> <small class="label label-primary">  <%=status%></small> <%=title %> </label>
                                    </div>
                           		 </div>
