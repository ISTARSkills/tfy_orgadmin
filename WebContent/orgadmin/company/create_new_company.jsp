<%@page import="com.istarindia.apps.dao.Company"%>
...
<% 
	...
	String company_id = request.getParameter("company_id");
		newCompany = false;
		o = (Company)new OrganizationDAO().findById(Integer.parseInt(company_id));
		}
		...
		<div id="page-wrapper" class="gray-bg ">
			...
			<div class="wrapper">
				<div class="vacancy_tabs_pane white-bg page-heading">
					<h2 style="padding-top: 15px;">Create or Update Company</h2>
				</div>

				<div class="panel-body" style="background-color:white;">
				
											<form method="post" class="form-horizontal" action="<%=baseURL%>update_company" enctype="multipart/form-data">
							
									<div class="form-group">									
										<label class="col-sm-2 control-label">Name</label>
										<div class="col-sm-6">
											<input type="text" class="form-control" placeholder="Company Name" name="name" data-validation="required" value="<%=!newCompany?o.getName():"" %>">
										</div>
									</div>
									...
									<div class="form-group">
										<label class="col-sm-2 control-label">Select Industry</label>
										<div class="col-sm-6">
											<select data-placeholder="Choose Industry" class="chosen-select form-control" name="industry_type"  tabindex="2" data-validation="required">
											<option value="<%=!newCompany?o.getIndustry():null %>" <%=!newCompany?"selected":"" %>><%=!newCompany?o.getIndustry():"Select Industry Type" %></option>
												<% ArrayList<String> industry_list = o.getIndustries(); 
													for(String industry_name: industry_list){%>
									...
									<div class="form-group">									
										<label class="col-sm-2 control-label">Company Logo Image</label>
										<div class="col-sm-6">
										<input type="file" placeholder="Upload Company Logo" data-validation="required" name="name" class="hidden upload_company_logo" accept=".png,.PNG,.JPG,.jpg,JPEG,.jpeg"/>
										<div class="">
										<img class="company_image" style="width: 100px; height: 100px; border: solid #f3f3f3; border-width: 1px;" src="<%=!newCompany?o.getImage():"/video/company_image/default.png"%>"/>
										</div>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-10 ttr">
											...
											<%=!newCompany?o.getCompany_profile():"" %>
											</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" class="form-control" data-validation="required" placeholder="Eg. Infosys Gate 1, 1st Main Rd" name="addressline1" value="<%=!newCompany?o.getAddress().getAddressline1():"" %>">
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-2 control-label">Address Line 2</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" data-validation="required" placeholder="Eg. Electronics City Phase 1, Electronic City" name="addressline2" value="<%=!newCompany?o.getAddress().getAddressline2():"" %>">
										</div>
									</div>
									<div class="form-group">
										...
										<div class="col-sm-10">
											...
											<option value="<%=!newCompany?o.getAddress().getPincode().getPin():"" %>" <%=!newCompany?"selected":"" %> style="width: 350px;" tabindex="2"><%=!newCompany?o.getAddress().getPincode().getPin():"Select Pincode" %></option>
										</select>
									...
									<input type="hidden" name="company_id" value="<%=request.getParameter("company_id").toString()%>">
									<%} %>
								</form>
...
<%-- 	<script src="<%=baseURL%>js/plugins/summernote/summernote.min.js"></script> --%>
	...
	<script>
	 ...
	 $('.company_image').on("click", function(){
		 $('.upload_company_logo').click();
	 });
	 
	$('.upload_company_logo').on("change", function(){
		readImage(this);
		 }) 
