...
<div class="tabs-container row" id="head_list">
		<ul class="nav nav-tabs row list_filter_tab list_filter_tab--<%=stage_id%>-<%=vacancyID%><%=collegeID%>" >
			<li class="active col-lg-10"><a data-toggle="tab" aria-expanded="true"
				href="#list_students--<%=stage_id%>-<%=vacancyID%><%=collegeID%>" style="border:none;">Students List</a></li>
		...
		<div class="row select_all_pane border-top">
				...
				<input type="checkbox" class="i-checks allCheck select_all_students-<%=stage_id%><%=vacancyID%><%=collegeID%>" data-stage="<%=stage_id%>" data-vacancy=<%=vacancyID%> data-college="<%=collegeID%>" data-checked="false"/>			
				</div>
		</div>
	...
	<div id="list_students--<%=stage_id%>-<%=vacancyID%><%=collegeID%>" class="tab-pane active" >
				...
				<div data-vacancy_id="<%=vacancyID%>"
					...
					%>
					<a href="<%=baseURL%>StudentPaginatorRecruiter?startIndex=15&stageID=<%=stage_id%>&vacancyID=<%=vacancyID%>&collegeID=<%=collegeID%>">next</a>	
				</div>
					<div class="promote_tab row" style="background-color: #e2e4e6; height:46px; bottom:0; margin-right:15px; padding-top: 7px;position:fixed;">
		<div style="padding-right: 15px;"><button type="button" data-loading-text="Promoting"
			class="btn btn-outline btn-primary pull-right prRomote_students"
