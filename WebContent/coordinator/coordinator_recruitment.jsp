				...
				<div class="row" id="filters">
					...
					<div class="big-demo go-wide" data-js="filtering-demo">
						<div class="filter-button-group button-group js-radio-button-group">
							<span class="badge badge-info">Filter by Course</span>
							<div class="row m-t-xs">
								<div class="col-md-10">
									<button class="btn btn-outline btn-info is-checked btn-xs" style="margin-bottom: 5px; margin-right: 5px" data-filter="*">show all</button>
									<%
										for (HashMap<String, Object> item : items) {
									%>
									<button class="btn btn-outline btn-info btn-xs" style="margin-bottom: 5px; margin-right: 5px" data-filter=".course_<%=item.get("course_name").toString().replaceAll("-", "_").replaceAll(" ", "_")
						.replaceAll("/", "___").trim()%>"><%=item.get("course_name").toString()%></button>
									<%
										}
									%>
								</div>
					...
					<div class="row" style="margin-top: 20px; margin-left: 15px;">
						<div class="big-demo go-wide" data-js="filtering-demo">
							<div class="filter-button-group button-group js-radio-button-group">
								...
								<div class="row m-t-xs">
									<div class="col-md-10">
										<button class="btn btn-outline btn-warning btn-xs is-checked" style="margin-bottom: 5px; margin-right: 5px" data-filter="*">show all</button>
										<%
											for (HashMap<String, Object> item : items1) {
										%>
										<button class="btn btn-outline btn-warning btn-xs" style="margin-bottom: 5px; margin-right: 5px" data-filter=".cluster_<%=item.get("cluster_name").toString().replaceAll("-", "_").replaceAll(" ", "_")
						.replaceAll("/", "___").trim()%>"><%=item.get("cluster_name").toString()%></button>
										<%
											}
										%>
									</div>
									...
									<tbody>
										...
										<%
											String findInterestedCourse = "select course.id , course.course_name from  trainer_intrested_course, course where trainer_intrested_course.trainer_id = "
															+ data.get(i).get("id") + " and  course.id = trainer_intrested_course.course_id";
													...
													if (courseData.size() > 0) {
														...
														for (HashMap<String, Object> courseRow : courseData) {
															...
															String getL3Result = "select cast (sum(score) as integer) as user_score, (select cast (count(assessment_question.id) as integer) as total from assessment_question where assessmentid in (select DISTINCT assessment_id from course_assessment_mapping where course_id ="
																	+ courseRow.get("id") + "))   from report where user_id = " + data.get(i).get("id")
															...
															if (scoreL3.size() > 0) {
																if (scoreL3.get(0).get("user_score") != null) {
																	L3 = scoreL3.get(0).get("user_score").toString() + "/"
								...
								<div class="show_more" style="display: none;">
									...
									<div class="row">
										<div class="col-lg-12">
											...
											%>
											<button class="btn btn-white btn-xs" type="button"><%=slot.replaceAll("- 9am, 9am -", "-").replaceAll("- 10am, 10am -", "-")
									.replaceAll("- 11am, 11am -", "-").replaceAll("- 12pm, 12pm -", "-")
									.replaceAll("- 1pm, 1pm -", "-").replaceAll("- 2pm, 2pm -", "-")
									.replaceAll("- 3pm, 3pm -", "-").replaceAll("- 4pm, 4pm -", "-")
									.replaceAll("- 5pm, 5pm -", "-")%></button>
											<br />
											<%
												}
											%>
									...
									<%
										if (data.get(i).get("cities") != null
														&& !data.get(i).get("cities").toString().equalsIgnoreCase("")) {
									%>
									<div class="row">
										<div class="col-lg-12">
											<h5>Cities:</h5>
