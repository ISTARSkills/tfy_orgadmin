...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<nav class="navbar navbar-static-top" role="navigation">
		...
		<div class="navbar-collapse collapse" id="navbar">
			...
			<ul class="nav navbar-nav">
				...
				<li class="dropdown"><a aria-expanded="false" role="button"
					...
					<ul role="menu" class="dropdown-menu">
						...
						%>
						<li><a id="<%=link.getDisplayName().replace(" ", "")%>"
							href="<%=child.getUrl()%>"><%=child.getDisplayName()%></a></li>
			...
			<ul class="nav navbar-top-links navbar-right" style="display: flex">
				<%
					if (loggedInRole.equalsIgnoreCase("TRAINER") || loggedInRole.equalsIgnoreCase("STUDENT")
							|| loggedInRole.equalsIgnoreCase("MASTER_TRAINER")) {
				%>

				<span class="label" style="margin-top: 14px; color: #eb384f; font-size: 15px; background: white;"><img
					alt="" src="/assets/img/user_images/coins_icon.png"
					style="width: 14px;"> <%=cp != null && cp.getStudentProfile() != null && cp.getStudentProfile().getCoins() != null
						? cp.getStudentProfile().getCoins()
						: "0"%></span>
				<span class="label" style="margin-top: 14px; color: #eb384f; font-size: 15px; background: white;"><%=cp != null && cp.getStudentProfile() != null
						&& cp.getStudentProfile().getExperiencePoints() != null
								? cp.getStudentProfile().getExperiencePoints()
								: "0"%>&nbsp;&nbsp; XP </span>

				<li class="dropdown"><a class="dropdown-toggle count-info"
					data-toggle="dropdown" href="#" aria-expanded="false"> <i
						class="fa fa-bell"></i> <span class="label label-primary"><%=cp != null ? cp.getNotificationsValid() : ""%></span>
				</a>
					<ul class="dropdown-menu dropdown-alerts"
						style="overflow: auto; max-height: 571px;">
						<%
						...
						%><li>
							<div
								class='row  dropdown-messages-box notification_holder_status'
								data-notifiction="<%=notification.getId()%>"
								data-url="<%=baseURL + "t2c/notifications/user/" + cp.getId()%>">
								<div class='col-md-2'>
									<img src="<%=notification.getImageURL()%>" class="img-rounded"
										style="width: 100%">
								</div>
								<div class='col-md-9'>
									<%=notification.getMessage().trim()%>
									<br /> <br /> <span class="pull-right text-muted small"><%=time%></span>
								</div>
							</div>
						</li>
						<li class="divider"></li>
				...
				<li><div
						style="border-left: 1px solid #e7eaec; height: 32px; margin-bottom: -11px; margin-left: 7px; margin-right: 7px;"></div></li>
				<%
					}
 ...
 if (userRoles.size() > 1 || userRoles.iterator().next().getRoleName().equalsIgnoreCase("SUPER_ADMIN")) { %>
 	...
 	for (Role role : userRoles) {
						...
						<li><a tabindex="-1" href="/role_switch_controller?user_id=<%=user.getId()%>&main_role=<%=mainRole%>&new_role=<%=role.getRoleName()%>">Switch to <%=role.getRoleName()%></a></li>
				...
				%>
				<img style="height: 34px !important; margin-top: 8px;"
					src="<%=imageurl%> " class="img-circle img-md">
