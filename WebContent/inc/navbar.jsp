...
<nav class="navbar navbar-expand-md fixed-top">
	...
	<div class="collapse navbar-collapse" id="navbarsExampleDefault">
		<ul class="navbar-nav mr-auto">
			...
			<li class="nav-item dropdown <%=activeClass%>"><a class="nav-link dropdown-toggle" href="<%=link.getUrl()%>" id="<%=link.getDisplayName().replace(" ", "").toLowerCase()%>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><%=link.getDisplayName()%></a>

				<div class="dropdown-menu" aria-labelledby="<%=link.getDisplayName().replace(" ", "")%>">
		...
		<ul class="navbar-nav">
			<li class="nav-item custom-leftmargin-type1"><a id="" class="nav-link custom-xp-number"><%=cp.getStudentProfile().getExperiencePoints()%> <small class='custom-xp'>XP</small></a></li>
			<li class="nav-item custom-leftmargin-type1"><a id="" class="nav-link custom-coins"><img class='custom-coin-icontag' src="/assets/images/coin-icon.png" class="rounded" alt=""><small class='custom-xp'> <%=cp.getStudentProfile().getCoins()%></small></a></li>

			<li class="nav-item dropdown custom-leftmargin-type1 custom-rightmargin-type1 pt-0 pb-0"><a class="nav-link dropdown-toggle pt-0 pb-0" id="navbarDropdownNotificatinMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"> <i class="fa fa-bell fa-4 mt-3 custom-bell-color" aria-hidden="true"></i><span class="badge badge-info custom-notificatin-bell"><%=cp.getNotifications().size() %></span></a>
				<div class="dropdown-menu scrollbar text-center pt-0 pb-0" aria-labelledby="navbarDropdownNotificatinMenuLink">
					...
					<a class="dropdown-item custom-textSize <%=np.getStatus().equalsIgnoreCase("READ")?"text-muted":"" %>" href="#">
						<div class='row p-0'>
							<div class='col-2 p-0'>
								<img class='custom-notification-imgtag' src="<%=np.getImageURL()%>" alt="">
							</div>
...
<%}%>
		...
		<ul class="navbar-nav custom-nav-left-border">
			<li class="nav-link" href="#"><img src="<%=cp.getStudentProfile().getProfileImage()%>" class="img-circle custom-profile-imgtag" alt=""></li>
			...
			%> <a class="nav-link dropdown-toggle custom-profil-name" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">Welcome <%=firstName.toUpperCase()%>
			</a>
				<div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
					<a class="dropdown-item" href="#">Profile</a> <a class="dropdown-item" href="/auth/logout">Logout</a>
				</div>
			</li>
